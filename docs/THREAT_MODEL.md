...
| **Evidence** | `npm run check` — typecheck, 209 tests, build. |
...
| **RR-2** | **Egress broker not built.** T4/T5 network confinement is detective, not topological. | Ran out of build time; specified in §4.4 of the Track C doc. | `KS-1` rules still deny at G3 | `--internal` network + broker sidecar |
| **RR-3** | **No live container run under the profile.** Isolation is asserted over generated argv. | Needs the daemon and runtime image. | 13 argv-level isolation tests | Run one subtask end to end |
...
```bash
npm run check          # typecheck + 209 tests + build
