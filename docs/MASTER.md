...
| **R17** | **VS Code workbench shell** | web | `@daniel` | `DONE` | 2026-08-31 | Replaces the Playground/Console split with one shell: title bar, activity bar, sidebar views, editor groups, tabbed bottom panel, status bar, command palette, quick open, keybindings. `console.css` hardcodes no colours, so the Dark+/Light+ palette is a token retarget in `styles.css:14-92`, not a rewrite. The baseline Create -> Start -> Chat journey moves intact into a Run view - G6 must not regress. |
| **R18** | **Monaco editor** replacing the hand-rolled viewer | web | `@daniel` | `DONE` | 2026-08-31 | `monaco-editor` + `@vscode/codicons`, no CDN loader, no `@monaco-editor/react`. Blame, comment glyphs, conflict regions and carets all become decorations. The editor owns no network call - the human write path is a prop seam (`onRequestSave`), owned by whoever builds human-in-the-loop editing. `Code.tsx`'s "no Monaco" header comment is rewritten, not dropped. |
| **R19** | **Commit carets** - character-level attribution | CONCORD | `@daniel` | `DONE` | 2026-08-31 | Claims `provenance.ts` and `store.ts`. `reconcileProvenance` already diffs previous vs next content inside the commit critical section; at the last inserted line of the last hunk, `lines.length` IS that line number, and the column is one string comparison away. So the caret is arithmetic over data CONCORD already holds - no file read, no new route. Carries on `AgentContribution` (durable) and `PresenceEntry` (15s TTL). **It marks where a commit ended, not where a keystroke is**, and it does not move during a turn: Codex reports items, not keystrokes. Interpolation stays unbuilt and gated. Supersedes the "Live cursors - refused" row in `AMOEBA_INSPIRATION_SCOPE.md` §5, which gets rewritten to say what backs it. |
| **R21** | **Playground folded into the workbench as the Agents view** | web | `@daniel` | `DONE` | 2026-08-31 | The starter kit's Create -> Start -> Chat journey now lives in the shell: `views/AgentsView.tsx` (list, create, start/stop, runtime status), `views/AgentChat.tsx` (the conversation, as an editor surface) and `views/CreateAgentDialog.tsx`, all on the workbench tokens. The handlers moved verbatim into `state/useAgents.ts` - same endpoints, same order - because G6 is zero regression and the cheapest way to keep that true is not to rewrite the logic. The classic Playground is retained behind **Go > Open the Classic Playground** as the no-regression safety net. Deleted the 57 `:root[data-theme="dark"]` Playground overrides in `styles.css` that existed only to give it a dark mode; the unlock screen's is kept because that is still on the main path. **Not done:** tokenizing the 135 hex literals in the Playground's light rules - hours of mechanical risk on a surface that is now legacy, so the classic Playground is light-only. |
| **R20** | **Stop a subtask Agent mid-turn** | WARRANT | `@jemy` | `DONE` | 2026-08-31 | Built in feat/multi-agent-v2 exactly as specified here: `POST /api/warrant/subtasks/:subtaskId/stop`, owner-checked like `/run`, returns the subtask to `assigned` and publishes a `blocked` activity event. Nothing is reconciled afterwards - an interrupted turn never reached a checkpoint, so offering its half-finished edits to CONCORD would commit work nobody approved. The Stop button is live, not disabled. Original note: there was no route for it. `POST /api/agents/:id/stop` is the CORE agent model (UUID, own workspace), not the subtask model (`agent_<uuid>`, minted by planning) - wiring one to the other would be a lie in the UI. Needed: `POST /api/warrant/subtasks/:subtaskId/stop`, owner-checked like `/run`, returning the subtask to `assigned` and publishing a `blocked` activity event. Until it exists the UI's Stop button renders disabled and says why. Blocks the human-in-the-loop story. |
| **R22** | **G3 should not kill a run on a heuristic** | AEGIS | `@___` | `TODO` | 2026-08-31 | **The one that matters.** `fs.read` requests are derived by scanning shell TEXT, and that is enough to contain a run. Shell text cannot be parsed reliably: five distinct false positives killed legitimate turns (globs, heredoc bodies, escaped slashes, relative paths, wedged quotes) - all fixed, but the class is open. Suggested shape: RECORD heuristic extractions, and reserve containment for `file_change` events, which Codex reports structurally. Policy design change - AEGIS owner's call. See [MULTI_AGENT_V2.md](MULTI_AGENT_V2.md) §6. |
| **R23** | **Flaky AEGIS test** | AEGIS | `@___` | `TODO` | 2026-08-31 | `guarded-runner.test.ts` fails ~1 full run in 8 (`KS-9 global kill switch`, or the breakers) and passes in isolation every time. The latch and breakers are process-global while the tests share one temp `dir`, so ordering leaks. Pre-existing. |
...
- `2026-08-31` · `@daniel` · **Run and Debug (R21), and a G6 regression caught by looking.** The Playground's journey now lives inside the workbench - create an Agent from a dialog, start it, chat to it in an editor tab - with the handlers moved verbatim into `state/useAgents.ts` so the endpoints and their order are untouched. The classic Playground is kept behind Go > Open the Classic Playground as the no-regression net. **The regression:** `workbench.css` had an unscoped `.sidebar` rule whose `grid-area: sidebar` also matched the Playground's own `<aside class="sidebar">`; that area name does not exist in `.app-shell`'s grid, so the browser invented columns and threw the Playground's side bar into a phantom track off-screen. Scoped to `.workbench > .sidebar`. It was found by opening the thing and looking at it, which is now three for three on bugs that no test caught.
- `2026-08-31` · `@daniel` · **Monaco, Source Control and commit carets (R18, R19), and R14 finally discharged.** The console has now been looked at in a browser, driven end to end, and the bugs that only running it could find are fixed: the side bar was painting through the status bar (grid items default to `min-height:auto`), and the Playground's untokenized `.sidebar`/`.main` rules were leaking into the workbench (now scoped under `.app-shell`). **Monaco** replaces the hand-rolled viewer - real highlighting, minimap, folding - with blame, comments, conflicts and carets all as decorations (`editor/decorations.ts`, pure functions). It owns no network call: saving is the `onRequestSave` prop, whose implementation must use the ordinary `POST /api/concord/docs/:docId`, so there is still exactly one write path. **Source Control** renders CONCORD revisions in VS Code's SCM shape from `/contributions` and `/history`, two routes no client had ever called, and the Explorer is now a real tree derived from doc ids. **Commit carets:** `reconcileProvenance` already diffs previous against next content to attribute lines, so the line and column where a commit ENDED are arithmetic over data CONCORD holds - no new route, no file read, no sampling. Carried on `AgentContribution.caret` (durable) and `PresenceEntry.caret` (15s TTL). Verified live: Bob's agent changed `100`→`250` on line 6 and the caret landed at column 54, one past the `5` - the trailing `0` is genuinely shared, which is the rule working, not a rounding. **It marks a commit, not a keystroke, and it never interpolates**; the refusals in `AMOEBA_INSPIRATION_SCOPE.md` §5 and the `HANDOFF.md` scorecard are rewritten to say exactly what backs it rather than quietly dropped. Also plumbed an optional `message` on the CONCORD write route, so a human's save can name its own commit instead of reading "n lines changed". 363 server tests + 8 web (**371**), `npm run check` green.
- `2026-08-31` · `@daniel` · **VS Code workbench shell, first half (R17).** The console is now a real workbench: 35px title bar with menus and a quick-input box, 48px activity bar on codicons (the icon font is vendored from `@vscode/codicons`, 80KB in the bundle - no CDN, so it works offline), a resizable side bar, an editor area, a five-tab bottom panel that now also holds Agent Live and the decision stream, and a status bar built from an item list rather than a fixed row. Plus a command palette and quick open (`Ctrl+Shift+P` / `Ctrl+P`) over one command registry, so every action the mouse can reach the keyboard can too. The Dark+/Light+ palette is a **token retarget**, not a rewrite: `console.css` hardcodes no hex, so reassigning ~40 declarations re-skinned 2051 lines for free. Sash drags write a CSS custom property instead of React state, so resizing never re-renders the workbench. 358 server tests still green, plus 8 new web tests for the keybinding matcher and the palette's fuzzy filter (**366**). Still to come: Monaco (R18) and commit carets (R19).
