...
describe("the platform runs as one operator", () => {
  it("seeds exactly one human", async () => {
    const humans = (
      await app.inject({ method: "GET", url: "/api/warrant/humans" })
    ).json().humans as { id: string; displayName: string }[];
    expect(humans).toHaveLength(1);
    expect(humans[0]?.id).toBe("human:orchestrator");
    expect(humans[0]?.displayName).toBe("You");
  });
});
