...
function AgentScreen({
  ...
  useEffect(() => {
    ...
    const list: monacoNs.editor.IModelDeltaDecoration[] = [];
    ...
    if (band) {
      if (band.start > 1) {
        list.push({
          range: new monacoNs.Range(1, 1, Math.min(band.start - 1, lineCount), 1),
          options: { isWholeLine: true, className: "screen-foreign" },
        });
      }
      if (band.end < lineCount) {
        list.push({
          range: new monacoNs.Range(band.end + 1, 1, lineCount, 1),
          options: { isWholeLine: true, className: "screen-foreign" },
        });
      }
      list.push({
        range: new monacoNs.Range(band.start, 1, Math.min(band.end, lineCount), 1),
        options: {
          isWholeLine: true,
    ...
    <div className={"screen" + (working ? " is-working" : "")}>
      ...
      <div className="screen-body">
        {frame ? (
          <Editor
            height="100%"
            language={languageOf(frame.docId)}
            value={frame.content}
            onMount={(editor, instance) => {
              editorRef.current = editor;
              defineThemes(instance);
              instance.editor.setTheme(
                theme === "dark" ? "concord-dark" : "concord-light",
              );
            }}
            options={{
              readOnly: true,
